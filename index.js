...
const cheerio = require('cheerio'); // (SMS side ke liye, untouched)
...
/* 🔹 SESSION (browser se uthao) */
const PHPSESSID =
  process.env.PHPSESSID || 'nus523do9hbsiakb3f28jqtjc5';
...
function getCountryFromNumber(number) {
  try {
    const num = number.toString().startsWith('+')
      ? number
      : '+' + number;
...
app.get('/api/numbers', async (_, res) => {
  try {
    if (
      cachedNumbers &&
      Date.now() - lastNumberFetch < NUMBER_CACHE
    ) {
